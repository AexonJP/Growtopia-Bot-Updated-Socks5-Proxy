...
void gui::Render() noexcept
{
	...
	auto test = XorStr(" SOCKS5 SUPPORT");
	ImGui::PushFont(font);
...
int main() {
	SetConsoleTitleA(XorStr("SOCKS5 Console").c_str());
	version();
	init();
	...
	const auto response = request.send("POST", "version=3.96&protocol=162&platform=0", { "Host: www.growtopia1.com" });
	rtvar var = rtvar::parse({ response.body.begin(), response.body.end() });
	var.serialize();
	if (var.find("server")) {
		SERVER_PORT = var.get_int("port");
